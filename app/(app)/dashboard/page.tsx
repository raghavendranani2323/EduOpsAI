...
export default async function DashboardPage() {
  ...
  return (
    <div className="px-4 pt-3 pb-4 md:p-6 space-y-3.5 md:space-y-6 max-w-2xl animate-fade-in">
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{institution.name}</p>
        <h1 className="text-xl md:text-[28px] leading-tight font-bold tracking-tight mt-0.5">{greeting()}</h1>
      </div>
      ...
      {(data.unmarkedToday > 0 || data.overdueInvoices > 0 || data.followupsDue > 0) && (
        <div className="space-y-2">
          <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Action needed</p>
      ...
      {!data.isTeacher && (
        <div className="grid grid-cols-2 gap-2">
          ...
          <Kpi icon={TrendingUp} label="Collected this month" value={formatINR(data.feeCollected)} hint={data.feeOutstanding > 0 ? `${formatINR(data.feeOutstanding)} pending` : undefined} valueClass="text-green-700 dark:text-green-300" />
...
function ActionLink({ href, icon: Icon, title, subtitle, tone }: { href: string; icon: React.ElementType; title: string; subtitle: string; tone: keyof typeof TONE_CLASSES }) {
  ...
  return (
    <Link
      href={href}
      className={`group flex items-center gap-3 border rounded-2xl p-2.5 md:p-3.5 transition-all active:scale-[0.99] hover:shadow-md ${c.wrap} ${c.text}`}
    >
      <div className={`h-9 w-9 md:h-10 md:w-10 rounded-xl flex items-center justify-center shrink-0 ${c.iconBg}`}>
  ...
  return (
    <Card className="p-3 md:p-4">
      <div className="flex items-center gap-1.5 text-muted-foreground mb-1.5">
        ...
        <span className="text-[10px] md:text-[11px] font-semibold uppercase tracking-wider truncate">{label}</span>
      </div>
      <p className={`text-xl md:text-[26px] leading-none font-bold tabular-nums tracking-tight truncate ${valueClass ?? ""}`}>{value}</p>
...
function QuickAction({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
  return (
    <Link
      href={href}
      className="group rounded-2xl border border-border bg-card px-1 py-2.5 hover:shadow-md hover:-translate-y-0.5 hover:border-primary/30 transition-all active:scale-[0.97] flex flex-col items-center gap-1.5"
    >
      ...
      <span className="text-[10.5px] font-semibold leading-tight text-center">{label}</span>
    </Link>
  );
}
